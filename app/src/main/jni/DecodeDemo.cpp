...
bool DecodeDemo::Init(JNIEnv *jniEnv, jobject surface) {
    string filename = "/sdcard/DCIM/HEVC.mp4";
    ...
    media_status_t status = AMediaCodec_configure(codec_, format_, m_ANativeWindow, nullptr, 0);
    if (status) {
    ...
    while (true) {
        ...
        if (!decoder_eos) {
            ...
            if (outputidx >= 0) {
                ...
                AMediaCodec_getOutputBuffer(codec_, outputidx, nullptr);
                AMediaCodec_releaseOutputBuffer(codec_, outputidx, true);
